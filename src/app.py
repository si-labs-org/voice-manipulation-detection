...
class InferenceEngine:
    ...
    def preprocess(self, waveform_np):
        waveform_np = _normalize_audio(waveform_np)
...
class SpoofDetectorApp:
    ...
    def _build_ui(self):
        self.f_heading = font.Font(family='Segoe UI', size=16, weight='bold')
        ...
        self.f_verdict = font.Font(family='Segoe UI', size=28, weight='bold')
        self.f_score = font.Font(family='Consolas', size=11, weight='bold')
        ...
        tk.Label(
            hdr, text='Voice Spoof Detector', font=self.f_heading, bg=BG, fg=TEXT,
        ).pack(side='left')
        ...
        self._dot(YELLOW)
        self.status_lbl = tk.Label(status_f, text='Loading model\u2026', font=self.f_mono, bg=BG, fg=TEXT_DIM)
        ...
        tk.Label(right_ctrl, text='Input:', font=self.f_label, bg=BG, fg=TEXT_DIM).pack(side='left', padx=(0, 4))
        ...
        self.device_menu['menu'].config(bg=BG3, fg=TEXT, activebackground=ACCENT, activeforeground='#fff')
        ...
        tk.Label(bar_row, text='Spoof probability', font=self.f_sub, bg=BG2, fg=TEXT, width=18, anchor='w').pack(side='left')
        ...
        self.score_lbl = tk.Label(bar_row, text='--', font=self.f_score, bg=BG2, fg=TEXT_DIM, width=7, anchor='e')
        self.score_lbl.pack(side='left')

        self.tag_lbl = tk.Label(bar_row, text='', font=self.f_btn, bg=BG2, fg=TEXT_DIM, width=12, anchor='w')
        self.tag_lbl.pack(side='left', padx=(10, 0))

        tk.Frame(res_frame, bg=BG2, height=10).pack()
        ...
        self.verdict_lbl = tk.Label(vi, text='AWAITING INPUT', font=self.f_verdict, bg=BG2, fg=TEXT_DIM)
        self.verdict_lbl.pack(pady=(6, 0))
        self.verdict_sub = tk.Label(vi, text='Record or load an audio sample', font=self.f_sub, bg=BG2, fg=TEXT_DIM)
    ...
    def _set_controls(self, on):
        st: Literal['normal', 'disabled'] = 'normal' if on else 'disabled'
        self.rec_btn.config(state=st)
        self.load_btn.config(state=st)
        self.device_menu.config(state=st)
    ...
    def _draw_wave(self, data, color=ACCENT):
        ...
        for i in range(w):
            ...
            pts_top.extend([i, mid - hi * mid * 0.85])
            pts_bot.extend([i, mid - lo * mid * 0.85])
        if len(pts_top) >= 4:
    ...
    def _populate_devices(self):
        ...
        for idx, name in devs:
            ...
            if default_idx is not None and idx == default_idx:
                chosen = label
        if chosen is None:
            chosen = next(iter(self.device_map.keys()))
    ...
    def _on_record(self):
        if not SD_OK:
            self._set_status('sounddevice missing', RED)
            return
    ...
    def _record_worker(self):
        try:
            ...
            audio = sd.rec(total, samplerate=SAMPLE_RATE, channels=1, dtype='float32', device=dev)
            for remaining in range(RECORD_SECS, 0, -1):
                self.q.put(('tick', remaining))
                time.sleep(1)
            sd.wait()
            data = audio.flatten().copy()
            rms = float(np.sqrt(np.mean(data ** 2)))
            if rms < MIN_RMS_THRESHOLD:
                self.q.put(('warn_low_signal', rms))
            data = _normalize_audio(data)
            self.q.put(('rec_done', data))
            self.q.put(('infer', data))
        except Exception as e:
            self.q.put(('error', f'Recording: {e}'))
        ...
        try:
            ...
            if data is None:
                try:
                    ...
                    raise RuntimeError(f'Cannot read file: {e2}')
    ...
    def _anim_bar(self, target, color, frame=0, total=30):
        ...
        cur = target * ease
        self.bar_fill.place(x=0, y=0, relheight=1.0, relwidth=max(0, cur))
        self.bar_fill.config(bg=color)
        ...
        try:
            while True:
                ...
                if msg == 'model_loaded':
                    ...
                    self.rec_status.config(text=f'Low signal (rms={payload:.4f})')
                elif msg == 'rec_done':
                    self._stop_anim()
                    self.rec_status.config(text='')
                    ...
                    threading.Thread(target=self._infer_worker, args=(payload,), daemon=True).start()
                elif msg == 'result':
